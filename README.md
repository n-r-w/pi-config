...
1. Copy the contents of this repository's `agent` directory to `~/.pi/agent`.
2. Configure `~/.pi/agent/agent-suite/model-aliases/config.json` for the models you use. By default, the configuration uses `openai-codex/gpt-5.6-xxx` models. See [Model alias recommendations](#model-alias-recommendations).
3. Install [pi-agent-suite](https://github.com/n-r-w/pi-agent-suite) by running `pi install npm:pi-agent-suite`.
4. Run `pi`.
5. Select a main agent with `/agents` or `Ctrl+Option+A`, for example `Auto`.
6. Ask the agent about the available tools and workflows.
